...
static const char kDemosaicComputeGlsl[] =
    ...
    "}\n"
    "/* Cooperative Bayer tile: every 8x8 workgroup owns an 8x8 block of\n"
    " * output pixels plus a 2-pixel halo on each side = 12x12 samples.\n"
    " * The 64 threads load ~2.25 samples each into shared memory once\n"
    " * per dispatch; the Malvar-He-Cutler kernel then reads PX(dx,dy)\n"
    " * from shared instead of issuing 13 SSBO reads per output pixel.\n"
    " * On GK20A the SSBO chases two rows above / below the centre had\n"
    " * cold L2 behaviour under preview load — the tile wipes that out\n"
    " * and folds the P1(a) per-sample divide into a single call per\n"
    " * loaded pixel rather than thirteen. */\n"
    "shared float tile[12][12];\n"
    "void main() {\n"
    "    uint lx = gl_LocalInvocationID.x;\n"
    "    uint ly = gl_LocalInvocationID.y;\n"
    "    uint tid = ly * 8u + lx;\n"
    "    int wgOrigX = int(gl_WorkGroupID.x * 8u);\n"
    "    int wgOrigY = int(gl_WorkGroupID.y * 8u);\n"
    "    int iw = int(params.width) - 1;\n"
    "    int ih = int(params.height) - 1;\n"
    "\n"
    "    /* Cooperative halo load — 144 samples, 64 threads, up to three\n"
